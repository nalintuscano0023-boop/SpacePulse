...
/**
 * Creates a clean, scientifically inspired, recognizable low-poly Voyager 1 spacecraft model.
 * 
 * Accurately features:
 * 1. 10-sided (decagonal) main equipment bus with gold thermal blanket material
 * 2. 3.7-meter parabolic high-gain antenna (HGA) with white/silver reflector, feed horn, and tripod struts
 * 3. RTG boom with 3 stacked cylindrical radioisotope thermoelectric generators (dark charcoal fins)
 * 4. Long deployable magnetometer (MAG) boom with low-field and high-field magnetometer sensor canisters
 * 5. Science scan platform boom with TV camera barrels, photopolarimeter, and spectrometers
 * 6. Dual V-shaped plasma wave / radio astronomy whip antennas
 * 7. The Golden Record phonograph mounted on the bus flank
 * 
 * Physical PBR materials with solid depth testing, no accidental transparency or fading.
 */
export function createVoyagerSpacecraftModel(): THREE.Group {
  ...
  // --- Realistic Physical Materials (Restrained, PBR, Solid Depth Testing) ---
  ...
  const dishMat = new THREE.MeshStandardMaterial({
    color: 0xf1f5f9, // Off-white / light silver reflector
    metalness: 0.25,
    roughness: 0.45,
  ...
  const titaniumMat = new THREE.MeshStandardMaterial({
    color: 0x64748b, // Titanium / graphite grey
    metalness: 0.7,
    roughness: 0.4,
    depthWrite: true,
    depthTest: true
  ...
  const darkStructureMat = new THREE.MeshStandardMaterial({
    color: 0x1e293b, // Charcoal matte composite
    metalness: 0.5,
    roughness: 0.8,
    depthWrite: true,
    depthTest: true
  ...
  const sensorMat = new THREE.MeshStandardMaterial({
    color: 0x0f172a, // Deep obsidian black optics
    metalness: 0.3,
    roughness: 0.2,
    depthWrite: true,
    depthTest: true
  ...
  // --- 1. Main Spacecraft Body: 10-sided (Decagonal) Equipment Bus ---
  // The actual Voyager bus is a decagonal prism holding electronics and hydrazine tanks.
  ...
  // Central fuel tank / propulsion cylinder on bottom
  ...
  // Hydrazine thruster clusters (small cones on bus perimeter)
  for (let i = 0; i < 4; i++) {
  ...
  // --- 2. High-Gain Antenna (HGA): 3.7m Parabolic Dish ---
  // Mounted directly on top of the decagonal bus, facing along +Y.
  ...
  const dishGeo = new THREE.SphereGeometry(
    ...
    Math.PI * 2,
    0,
    Math.PI * 0.36 // Shallow parabolic cap
  );
  const dishMesh = new THREE.Mesh(dishGeo, dishMat);
  dishMesh.position.y = 0.22;
  dishMesh.rotation.x = Math.PI; // Face upward (+Y)
  dishMesh.scale.set(1.0, 0.55, 1.0); // Flatten to parabolic profile
  dishMesh.castShadow = true;
  voyagerGroup.add(dishMesh);

  // Central feed horn hub inside the dish
  ...
  // --- 3. RTG Boom (Radioisotope Thermoelectric Generators) ---
  // Extends laterally to the left (-X axis), carrying 3 nuclear power units.
  ...
  // 3 Stacked Cylindrical RTGs with cooling fins
  for (let r = 0; r < 3; r++) {
    ...
    // Subtle thermal fin rings around each RTG
  ...
  // --- 4. Magnetometer Boom (MAG Boom) ---
  // The famous 13-meter deployable fiberglass lattice boom, extended along (+X, slightly angled back).
  ...
  // Low-Field Magnetometer (mid-span sensor canister)
  ...
  // High-Field Magnetometer (boom tip sensor canister)
  ...
  // --- 5. Science Scan Platform Boom ---
  // Articulated platform holding optical cameras and spectrometers, pointing along (+Z / forward).
  ...
  // Instrument Platform Base Housing
  ...
  // Narrow-Angle TV Camera Barrel (Long focal length)
  ...
  // Wide-Angle TV Camera Barrel (Shorter focal length)
  ...
  // Infrared Interferometer Spectrometer (IRIS) Housing
  ...
  // Photopolarimeter (PPS) & Ultraviolet Spectrometer (UVS)
  ...
  // --- 6. Plasma Wave (PWS) & Planetary Radio Astronomy (PRA) Whip Antennas ---
  ...
  // Record Cover Protective Shield Rim
  ...
  // Scale entire group to a convenient unit size
